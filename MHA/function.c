...
static void
_local_gemm_rcr(const float *A, const int LDA, const float *B, const int LDB, float *C, const int LDC, int M, int N,
                int K) {
    for (int i = 0; i < M; i++)
        for (int j = 0; j < N; j++)
            for (int k = 0; k < K; k++)
...
static void
_local_gemm_rrr(const float *A, const int LDA, const float *B, const int LDB, float *C, const int LDC, int M, int N,
                int K) {
    for (int i = 0; i < M; i++)
        for (int j = 0; j < N; j++)
            for (int k = 0; k < K; k++)
...
static void
_local_gemm_rrr2(const float *A, const int LDA, const float *B, const int LDB, float *C, const int LDC, const int LDC2, int M, int N,
                int K) {
    for (int i = 0; i < M; i++)
        for (int j = 0; j < N; j++)
            for (int k = 0; k < K; k++)
                C[i * LDC + j + LDC2] += A[i * LDA + k] * B[k * LDB + j];
}
...
int naive_multihead_attention(Args_t arg) {
    ...
    for (int b = 0; b < B; b++) {
        for (int n = 0; n < N; n++) {
            _local_gemm_rcr(x + b * S * D, D, w + n * PD, PD, Q + b * S * D + n * PD, PD, S, PD, D);
            _local_gemm_rcr(x + b * S * D, D, w + D * D + n * PD, PD, K + b * S * D + n * PD, PD, S, PD, D);
            _local_gemm_rcr(x + b * S * D, D, w + 2 * D * D + n * PD, PD, V + b * S * D + n * PD, PD, S, PD, D);
        }
    }
